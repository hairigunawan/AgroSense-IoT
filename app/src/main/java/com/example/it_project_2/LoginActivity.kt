...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val sharedPreferences = try {
            ...
            // Hapus file preferensi menggunakan API yang direkomendasikan
            // agar cache di memori dan file backup (.bak) juga ikut terhapus
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                deleteSharedPreferences("app_settings_encrypted")
            } else {
                val file = java.io.File(filesDir.parent + "/shared_prefs/app_settings_encrypted.xml")
                if (file.exists()) file.delete()
            }
            
            // Hapus Master Key dari Keystore
            ...
            // Coba buat ulang dengan kunci baru
            val masterKey = androidx.security.crypto.MasterKey.Builder(this)
                .setKeyScheme(androidx.security.crypto.MasterKey.KeyScheme.AES256_GCM)
                .build()
        ...
        val hasSavedCredentials = BiometricCredentialStore.hasPasswordCredentials(this)
        ...
        biometricPrompt = BiometricPrompt(this, executor,
            object : BiometricPrompt.AuthenticationCallback() {
                ...
                override fun onAuthenticationSucceeded(result: BiometricPrompt.AuthenticationResult) {
                    ...
                    val savedCredentials = BiometricCredentialStore.getPasswordCredentials(this@LoginActivity)

                    if (savedCredentials != null) {
                        setLoading(true)
                        auth.signInWithEmailAndPassword(savedCredentials.email, savedCredentials.password)
                            .addOnCompleteListener(this@LoginActivity) { task ->
                                setLoading(false)
                                if (task.isSuccessful) {
                                    val user = auth.currentUser
                                    if (user?.isEmailVerified == true) {
                                        Toast.makeText(baseContext, "Autentikasi biometrik berhasil!", Toast.LENGTH_SHORT).show()
                                    ...
                                    Toast.makeText(baseContext, "Sesi kedaluwarsa. Harap login kembali dengan password.", Toast.LENGTH_LONG).show()
                                }
                            }
                    } else {
                        Toast.makeText(applicationContext, "Login sidik jari belum siap. Silakan login ulang dengan email dan password terlebih dahulu.", Toast.LENGTH_LONG).show()
                    }
                }
        ...
        btnBiometric.setOnClickListener {
            ...
            when (biometricManager.canAuthenticate(BiometricManager.Authenticators.BIOMETRIC_STRONG or BiometricManager.Authenticators.DEVICE_CREDENTIAL)) {
                BiometricManager.BIOMETRIC_SUCCESS -> {
                    val savedCredentials = BiometricCredentialStore.getPasswordCredentials(this)
                    if (savedCredentials != null) {
                        biometricPrompt.authenticate(promptInfo)
                    } else {
                        Toast.makeText(this, "Sidik jari hanya tersedia setelah Anda login dengan email dan password pada akun ini.", Toast.LENGTH_LONG).show()
                    }
                }
                ...
                .addOnCompleteListener(this) { task ->
                    ...
                    if (task.isSuccessful) {
                        ...
                        if (user?.isEmailVerified == true) {
                            ...
                            val credentialsSaved = BiometricCredentialStore.savePasswordCredentials(this, email, password)
                            if (!credentialsSaved) {
                                Toast.makeText(baseContext, "Login berhasil, tetapi penyimpanan kredensial sidik jari gagal.", Toast.LENGTH_LONG).show()
                            } else {
